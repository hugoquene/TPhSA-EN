# Segmenting and labeling speech sounds {#ch-segmentinglabeling}
...
Figure \@ref(fig:window-soundeditor-again) illustrates the following three^[You may also notice the F2 locus of the [d] in *window*, at 2.118 s and ~1800 Hz, see §\@ref(sec:readspectrogram), voiced consonants.] characteristics of casual, fluent speech:

* Boundaries between speech segments are not clearly present, due to coarticulation, e.g. between the vowel and the final consonant of [smɔɫ] *small*.
...
## How to segment and label speech sounds in `Praat`
...
(4) Choose the button labeled `View & Edit`; this will open a TextGrid Editor window, which is quite similar to the SoundEditor window (see  §\@ref(sec:praatsoundeditor)).
...
::: {#box-windowsfile-details-3 .smallprintbox}
