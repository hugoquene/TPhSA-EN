...
Now that we know about spectra (§\@ref(sec:spectrum)) and about the source-filter theory of speech production (§\@ref(sec:sourcefilter)), we can discuss *Linear Predictive Coding* or *LPC* (key references are: @Markel_Gray_1976, @Atal_Schroeder_1978). 
...
[^ch08lpc-2]: The curved shape of the GREEN contour in Figure XX is due to the linear scale on the frequency axis, combined with the logarithmic shape of the filter envelope (octave).
...
The LPC inverse filtering as described above was performed on a brief windowed portion or *frame* of the speech signal. Traditionally this was done over 256 samples, or 25.6 ms of speech if sampled at $f_s = 10\  \textrm{kHz}$ (§\@ref(sec:samplingfrequency)). Within this frame, LPC assumes correctly that the filter is stable, as the vocal tract does not change much during 25.6 ms, and it assumes that the fine spectral details (e.g. harmonics) must have been caused by the source. Thus the source and filter characteristics are separated for this brief time frame. Next, the analysis frame is moved forward, typically by 10 ms (`frame rate`), and the analysis repeats for the next frame.
...
- As indicated above, LPC assumes that the *source and filter are two independent systems* (§\@ref(sec:sourcefilter)). 
...
- In traditional implementations, LPC will find a fixed (but user-settable) number of spectral filter coefficients. Each formant is represented by two of these LPC coefficients, which can be interpreted as its peak frequency and its bandwidth (§\@ref(sec:formants)). The *chosen number of formants in the LPC analysis must match the sampling frequency*, and it is the task of you, the analyst, to ensure that these properties do match. 
For adult male speakers, the formants are spaced about 1000 Hz apart (§\@ref(sec:formants)). So if you expect\|want to find 5 formants (10 spectral LPC coefficients) in male speech, the frequency range should be 0 to 5000 Hz, and the corresponding sampling frequency should be twice the upper limit, hence $f_s = 5 \times 1000 \times 2 = 10000$ Hz (see §\@ref(sec:samplingfrequency) for the $\times 2$ part of this rule). For adult female speakers, the formants are spaced about 1100 Hz apart. If you expect\|want to find 5 formants in female speech, the corresponding sampling frequency should be $f_s = 5 \times 1100 \times 2 = 11000$ Hz.
  ...
  - by choosing the *number of formants* in the LPC analysis proportional to the sampling frequency: the number of formants should be $f_s/2/1000$ for male speech and $f_s/2/1100$ for female speech;

  - by setting the *sampling frequency* $f_s$ (§\@ref(sec:samplingfrequency)) proportional to the desired number of formants, that is, by *downsampling* the speech signal to an $f_s$ that is appropriate for the desired number of formants. (For Figure  \@ref(fig:a8000lpc), the speech was downsampled to 8 kHz before LPC analysis, as there were only 4 formants noticeable in the original vowel from a male speaker.)\
  ...
  Remember that the number of coefficients in the LPC analysis is twice the number of formants, as _two_ coefficients (peak frequency and bandwidth) are required to describe _one_ formant in the LPC spectrum (BLUE). 

::: {#box-praatlpc .praatbox}

## How to perform LPC analysis in Praat
