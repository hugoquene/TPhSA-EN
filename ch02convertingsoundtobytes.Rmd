...
The amplitude resolution, or quantization, refers to the number of separate steps in amplitude (voltage) that are discerned during sampling. Again, with a higher amplitude resolution, the digital signal better (more closely) approximates the analog source in the amplitude dimension, resulting in a better digital recording. The amplitude resolution is expressed in bits^[1 bit is 2 possible values, `0` or `1`.] or bytes^[1 byte is 8 bits, or $2^8=256$ possible values.].
...
maximale uitsturing, zonder oversturing
...
- **While recording, check the level of the recording**. \
...
Make sure that your recording level is in the yellow zone, with occasional peaks in the red zone, but without clipping. 
...
- To do so, in the `Praat` object window, select the Sound object that you just recorded. 
