...
\author{true}
\date{Document compiled 05 Jul 2026 22:57}
...
\protect\phantomsection\label{question-intro}
...
\protect\phantomsection\label{tech-layout}
...
\item
  II: speech (phonetics),
\item
  III: speech technology -- under construction.
\end{itemize}
...
Please cite this work as follows (substitute the date):

Quené, H. (2026). \emph{Tutorial on Phonetics and Speech Analysis}. Retrieved 18 May 2026 from \url{https://hugoquene.github.io/TPhSA-EN} .
...
The 2007 version of the tutorial was subsequently revised and adapted to the current version using \texttt{R\ Markdown} \citep{rmarkdown2018} and \texttt{bookdown} \citep{R-bookdown} in \href{https://www.rstudio.com}{Rstudio} by Hugo Quené in 2024--2026. Relevant parts of older lab instructions were integrated in the main text of the present tutorial guide, where appropriate. These older instructions were written or revised earlier by a.o. Gerrit Bloothooft, Peter Pabon, Willemijn Heeren, Silvia Terenghi, Aleksei Nazarov, and Sanne Ditewig.
...
\protect\phantomsection\label{questions-soundwaves}
...
{\centering \includegraphics[width=22.92in]{figures/speech_word_oscillogram} 
...
\protect\phantomsection\label{box-speechfile-details}
...
\protect\phantomsection\label{log1}
...
Sound intensity or sound pressure is not expressed in absolute units, but in relative decibel units. The \emph{decibel} unit of intensity (or of sound pressure) is based on three steps \citep[p.91ff]{Fry_1979}:
...
\protect\phantomsection\label{questions-0dB}
...
Table \ref{tab:decibels} shows some examples of sound intensities under various circumstances \citep[p.94]{Fry_1979}.
...
\bottomrule\noalign{}
\endlastfoot
191 & Loudest sound possible in air, rarefactions are vacuum (\url{https://en.wikipedia.org/wiki/Sound_pressure\#Examples_of_sound_pressure}) \\
172 & Krakatoa eruption, 1893 (at \textasciitilde160 km distance) \citep[p.43]{Henderson_2023} \\
...
14 & Background noise inside recording cabin at Utrecht University {[}\url{https://ils-labs.wp.hum.uu.nl/cabins/cabin-2}{]} \\
...
\protect\phantomsection\label{questions-phase}
...
\protect\phantomsection\label{questions-wavelength}
...
\protect\phantomsection\label{box-praatintro}
...
\protect\phantomsection\label{box-open}
...
\protect\phantomsection\label{scientificnotation}
...
\protect\phantomsection\label{box-draw}
...
\protect\phantomsection\label{mic-howto}
...
\protect\phantomsection\label{box-nyquist}
...
\protect\phantomsection\label{box:telephonebandwidth}
...
\protect\phantomsection\label{recording}
...
\begin{itemize}
\item
  by adjusting the level of the input channel (perhaps called \texttt{Gain}), in your computer settings,
\item
...
\protect\phantomsection\label{box-praatrecord}
There is a helpful option in \texttt{Praat}, from the \emph{main} menu (not from the Objects window menu): \texttt{Praat\ \textgreater{}\ Settings\ \textgreater{}\ Sound\ recording\ settings...} where you may adjust certain global settings as needed.
...
\protect\phantomsection\label{box-praatplay}
...
\protect\phantomsection\label{box-praatscale}
...
\protect\phantomsection\label{box-segmenting}
...
\protect\phantomsection\label{box-soundeditor}
\begin{itemize}
\item
...
{\centering \includegraphics[width=31.31in]{figures/asmallwindowtogetthis_SoundEditor2050124} 
...
\protect\phantomsection\label{box-windowsfile-details}
...
\protect\phantomsection\label{fades}
...
\protect\phantomsection\label{box-praatfades}
...
\protect\phantomsection\label{box-praatconcatenate}
...
\emph{Chapter keywords}: sinewave sound, complex sound, spectrum, fourier transform, fourier analysis, fast fourier transform, FFT, harmonics, fundamental, fundamental frequency, f0, pitch, overtone, component, timbre, octave, noise, white noise, brown noise, impulse.
...
{\centering \includegraphics[width=50in]{figures/complex100n200n400} 
...
\protect\phantomsection\label{questions-spectrum}
...
\protect\phantomsection\label{box:praatspectrum}
...
\protect\phantomsection\label{box-praatspectralslice}
...
{\centering \includegraphics[width=20.61in]{figures/Spectrum_speech_word_0_210} 
...
{\centering \includegraphics[width=50in]{figures/whitebrownnoises} 
...
{\centering \includegraphics[width=17.78in]{figures/ADSR_parameter} 
...
An envelope is described by four properties:

\begin{itemize}
\item
  \emph{attack} (A): the ``onset time'' during which the property increases after the key has been pressed (in case of a piano key: the time between key-press and the hammer hitting the piano string and the string then starting to produce sound);
\item
  \emph{decay} (D): the time during which the property falls back from the peak of the attack (for a piano: the time from the instant of maximum sound) to the \ldots{}
\item
  \emph{sustain} (S): the \emph{level} of the property after the attack and decay phases, while the key is still pressed (for a piano: the level of sound from the string vibrating shortly after it was hit);
\item
  and \emph{release} (R): the time during which the property falls back from the sustain level, \emph{after} the key has been released (for a piano: very brief, unless the pedal is used).
\end{itemize}
...
\protect\phantomsection\label{box-praatemphasis}
...
{\centering \includegraphics[width=45.83in]{figures/onetube} 
...
where \(n\) is the ranking number of the natural frequency, \(c\) is the speed of sound (353 m/s, see §\ref{sec:speedofsound}), and \(L\) is the length of the tube \citep[p.139]{Stevens_1998} \footnote{And the corresponding \emph{wavelengths} of these natural frequencies are \(\frac{4}{2n-1}\cdot L\) \citep[p.60]{Nooteboom_Cohen_1984}.}.
...
{\centering \includegraphics[width=8.15in]{figures/formants_stevens01_b} 
...
{\centering \includegraphics[width=45.83in]{figures/twotube} 
...
{\centering \includegraphics[width=41.67in]{figures/speech_word_0_210_spectrum} 
...
\protect\phantomsection\label{box-praatformants}
\subsection{How to measure formants}\label{sec:measureformants}

The safest method for measuring formants is to \emph{bypass} the spectrum, and measure formants directly from a Sound object (this is further explained in Ch.\ref{ch-LPC}).

\begin{itemize}
\item
  Select an input Sound object in the \texttt{Praat} Objects window. Then choose \texttt{Analyse\ spectrum\ \textgreater{}} and then \texttt{To\ formant\ (burg)...}.
\item
  As we saw above, for a male speaker we may expect 5 formants in the range from 0 to 5000 Hz. For a female speaker, having a slightly shorter vocal tract than a male speaker, we may expect 5 formants in the range from 0 to 5500 Hz. Choose appropriate \texttt{Maximum\ number\ of\ formants} and choose the matching \texttt{Formant\ ceiling} value for your speaker; these values depend on the length of the speaker's vocal tract and on your sampling frequency (for further explanation, see §\ref{sec:LPC-details} and §\ref{sec:samplingfrequency} respectively).
  \footnote{Also the \texttt{Help} information in the \texttt{Praat} menu, in particular its last paragraph.}.
...
\protect\phantomsection\label{box:formantanalysis}
...
{\centering \includegraphics[width=14.22in]{figures/source-filter-theory34-l} 
...
{\centering \includegraphics[width=45.83in]{figures/merel-spectrogram} 
...
{\centering \includegraphics[width=50in]{figures/speech_word_spectrogram_5ms} 
...
{\centering \includegraphics[width=50in]{figures/speech_word_spectrogram_30ms} 
...
\protect\phantomsection\label{box-praatspectrogram}
...
\protect\phantomsection\label{box-spectrogram}
...
\protect\phantomsection\label{box-soundeditor-3}
\begin{itemize}
\item
  Select an input Sound object in the \texttt{Praat} Objects window.
\item
...
{\centering \includegraphics[width=28.31in]{figures/s321f1-1_informal_fragment_SoundEditor20250203} 
...
\protect\phantomsection\label{box-windowsfile-details-again}
...
\protect\phantomsection\label{box:clearinfo}
...
\protect\phantomsection\label{box-windowsfile-details-second}
...
\protect\phantomsection\label{box-praattextgrid}
...
\protect\phantomsection\label{box-praattextgrideditor}
...
{\centering \includegraphics[width=42in]{figures/asmallwindowtogetthisright_TextGridEditor20250130} 
...
\protect\phantomsection\label{box-windowsfile-details-third}
...
\chapter{Linear Predictive Coding}\label{ch-LPC}

\emph{Chapter keywords}: Linear Predictive Coding, source-filter theory, source, filter,
...
Now that we know about spectra (§\ref{sec:spectrum}) and about the source-filter theory of speech production (§\ref{sec:sourcefilter}), we can discuss \emph{Linear Predictive Coding} or \emph{LPC}. While this analysis method is now over 50 years old (key references are \citet{Atal_Hanauer_1971}, \citet{Atal_Schroeder_1978}, \citet{Markel_Gray_1976}), it is still highly relevant and important today (\citet{Story_2026}) in speech research and telecommunication.
...
\section{Inverse filtering}\label{sec:inversefiltering}

LPC works in the time domain (typically over a brief windowed portion of the input signal), but it is easier to explain in the frequency domain. \citet{Story_2026} offers more detailed explanation and background.

\begin{figure}

{\centering \includegraphics[width=50in]{figures/aa_8000_lpc} 

}

\caption{Spectrum of an [a] vowel, overlaid with (green) the assumed spectral slope of the glottal source, (red) the inverse LPC filter, and (blue) the resulting LPC spectral envelope}\label{fig:a8000lpc}
\end{figure}

To start, let us have a look at a spectral slice of a \emph{vowel} {[}a{]}, drawn in BLACK in Figure \ref{fig:a8000lpc} (see also §\ref{sec:spectralslice}).
At the left we see the first harmonic, the fundamental frequency, at \(f_0 = 108.5\,\textrm{Hz}\). The other harmonics are multiples of the fundamental (see §\ref{sec:FTintro}): the \emph{frequencies} of the harmonics are therefore entirely redundant, once we know that this is a voiced (periodic) sound with this \(f_0\). These two relevant LPC parameters are named \texttt{F0} and \texttt{VUV} (voiced\textasciitilde unvoiced). We also need to know the overall \texttt{Gain} (amplitude) of the input signal.

Moreover, we also assume that the glottal (voice) source sound itself has a particular spectral shape, \emph{before} it enters into the filter (vocal tract). Presumably, the overall spectral slope of the glottal (voice) source sound is \(-6\, \textrm{dB} / \textrm{octave}\), here drawn in GREEN \footnote{The glottal source signal has a presumed slope of \(-12\, \textrm{dB} / \textrm{octave}\). This spectral slope is combined with the effect of the radiation of the speech sound at the lips. Radiation or spreading of sound works better for higher frequencies than for low frequencies (that is one of the reasons why loudspeakers for low frequencies tend to be larger and consume more energy than those for high frequencies): this radiation effect is \(+6\, \textrm{dB} / \textrm{octave}\). For faster computation, we combine the effect of radiation into that of the glottal source, then the combined spectral slope is \(-12+6 = -6\) \(\textrm{dB} / \textrm{octave}\), and then we may subsequently ignore the radiation effect.} \footnote{The curved shape of the GREEN contour in Figure \ref{fig:a8000lpc} is due to the linear scale on the frequency axis, combined with the logarithmic shape of the filter envelope (octave).}.

However, what is \emph{not} expected from the glottal source sound is (are) the major peaks in the spectral envelope at approximately 800, 1450 and 3000 Hz. As we now know, those peaks are caused by the acoustic filter, that is, by formant resonances in the vocal tract (§\ref{sec:sourcefilter}, §\ref{sec:formants}).
...
Finally, as a check, we can pipe the \emph{presumed} glottal source signal (GREEN) with the proper \texttt{F0} and \texttt{Gain} into the filter, and compare the output of the filter (the predicted output speech, BLUE) with the original (BLACK). The difference between these two is termed the \emph{residual}
\footnote{The actual procedure is computationally different but conceptually similar.}.
If all has gone well, then according to the source-filter theory, the prediction should come out close to the original, and then the residual should contain only little information (buzz, noise or silence). In practice, however, the residual often contains salient phonetic properties of the speech. One reason for this is that the source and the filter are presumed to be independent in theory (e.g.~formant frequencies are independent of \texttt{F0}) but this assumption does not always hold. Female voices in particular tend to have stronger coupling or interference between source and filter than male voices (\citet{Titze_2008}, \citet{Klatt_Klatt_1990}).

The LPC inverse filtering as described above is performed on a brief windowed portion or \emph{frame} of the speech signal. Traditionally this was done over 256 samples, or 25.6 ms of speech if sampled at \(f_s = 10\  \textrm{kHz}\) (§\ref{sec:samplingfrequency}). Within this frame, LPC assumes correctly that the filter is stable, as the vocal tract does not change much during 25.6 ms, and it assumes that the fine spectral details (e.g.~harmonics) must have been caused by the source. Thus the source and filter characteristics are separated for this brief time frame. Next, the analysis frame is moved forward, typically by 10 ms (\texttt{frame\ rate}), and the analysis repeats for the next frame.

In this section, we have described LPC analysis (inverse filtering) for voiced vowels. This is because the source-filter theory applies better to vowels than to consonants. In the case of \emph{unvoiced consonants}, similar principles do apply, using a similar conceptualisation. For unvoiced consonants, the LPC analysis assumes that the source sound is white noise (§\ref{sec:noise}) generated somewhere in the vocal tract, with a flat spectral envelope. In \emph{voiced consonants}, \emph{two} source sounds are produced simultaneously: both the periodic phonation sound at the glottis, and the aperiodic noise at the constriction or closure. LPC has problems accounting for these two effects at the same time, and consequently, the residual still contains a lot of phonetic information: voiced consonants thus are rather unsuitable for LPC analysis. Consequently, the formants identified by LPC in consonants are less reliable than those in vowels.

\section{Limitations and considerations}\label{sec:LPC-details}

\begin{itemize}
\item
  As indicated above, LPC models the spectral envelope by a \emph{set of formant-like peaks}, where frequencies are amplified. LPC cannot represent so-called anti-formants, where frequencies are actively attenuated. These antiresonances do occur in nasals, nasalized vowels, laterals, and fricatives. LPC is not very suitable for the spectral analysis (nor the transmission) of these categories of speech sounds.
\item
  As indicated above, LPC assumes that the \emph{source and filter are two independent systems} in series (§\ref{sec:sourcefilter}).
  If there is \emph{interaction} between the source signal and the filter then the resulting LPC coefficients may be incorrect. This may happen between \texttt{F0} and the first formant in high vowels, in particular for women and children, or it may happen between formants (e.g.~\texttt{F1} and \texttt{F2} in {[}u{]}) (\citet{Titze_2008}).
\item
  ...
  The \emph{chosen number of formants in the LPC analysis must match the sampling frequency}, and it is the task of you, the analyst, to ensure that these properties do match.
  For adult male speakers, the formants are spaced about 1000 Hz apart (see §\ref{sec:formants}). So if you expect\textbar want to find 5 formants (10 spectral LPC coefficients) in male speech, the frequency range should be 0 to 5000 Hz, and the corresponding sampling frequency should be twice the upper limit, hence \(f_s = 5 \times 1000 \times 2 = 10000\) Hz (see §\ref{sec:samplingfrequency} about sampling frequency, esp.~for the \(\times 2\) part of this rule)\footnote{In other words, the number of LPC coefficients should be the sampling frequency \(f_s\) in kHz (\citet{Story_2026})}.
  For adult female speakers, the formants are spaced about 1100 Hz apart. If you expect\textbar want to find 5 formants in female speech, the corresponding sampling frequency should be \(f_s = 5 \times 1100 \times 2 = 11000\) Hz.
  ...
  \begin{itemize}
  \item
    by choosing the \emph{number of formants} in the LPC analysis proportional to the sampling frequency: the number of formants should be \(f_s/2/1000\) for male speech \footnote{or: the number of LPC coefficients should be \(f_s/1000\) or \(f_s\) in kHz, as suggested above.} and \(f_s/2/1100\) for female speech;
  \item
    by setting the \emph{sampling frequency} \(f_s\) (see §\ref{sec:samplingfrequency}) proportional to the desired number of formants, that is, by \emph{downsampling} the speech signal to an \(f_s\) that is appropriate for the desired number of formants. (For Figure \ref{fig:a8000lpc}, the speech was downsampled to 8 kHz before LPC analysis, as there were only 4 formants noticeable in the original vowel from a male speaker.)\\
  \end{itemize}

  If you fail to respect this relation between number of formants and sampling frequency, then the LPC analysis may result in spurious formants (a spectral peak is incorrectly considered to be a formant\footnote{Such spurious formants may have a very wide bandwidth, and thus a very low quality.}) or in missed formants (so that e.g.~the actual F4 is reported as \texttt{F3} because the true F3 has been missed).
  ...
  In more advanced implementations, as in \texttt{Praat} (\texttt{To\ Formant\ (burg)...}), the number of formants reported is independent of the sampling frequency, and such ``formant confusion'' is kept under control.

  Remember that the number of coefficients in the LPC analysis is twice the number of formants, because \emph{two} coefficients (peak frequency and bandwidth) are required to describe \emph{one} formant in the LPC spectrum (BLUE).
\end{itemize}

\protect\phantomsection\label{box-praatlpc1}
\section{How to perform LPC analysis}\label{how-to-perform-lpc-analysis}

\subsection{LPC analysis}\label{sec:LPC-analysis}

\texttt{Praat} has several algorithms to perform an LPC analysis. The classical LPC algorithm as described above uses the \texttt{autocorrelation} method (\citet{Markel_Gray_1976}). For phonetic analysis, where you typically want to interpret the LPC coefficients in terms of formants (frequencies and bandwidth), the \texttt{burg} method is more appropriate (and it is the recommended method in \texttt{Praat}); here we will indeed follow the \texttt{burg} method.

\protect\phantomsection\label{box:bypassLPC}
If you are only interested in the formant values, then the recommended workflow is to entirely \emph{bypass the LPC analysis} as described in this chapter. Instead, go directly from Sound to Formants, as explained in §\ref{sec:measureformants}.
\texttt{Praat} will then take care of the limitations and considerations and complications in the LPC analysis, generally with good results. However, if your formant analysis yields unexpected or implausible results, then the LPC considerations in this chapter may help you find solutions.

\protect\phantomsection\label{box-praatlpc2}
\begin{itemize}
\item
  Select an input Sound object in the \texttt{Praat} Objects window.
\item
  Choose \texttt{Analyze\ Spectrum...\ \textgreater{}\ To\ LPC...\ \textgreater{}\ To\ LPC\ (burg)...}.

  \begin{itemize}
  \item
    \texttt{Prediction\ order} refers to the desired number of LPC coefficients, which is twice the number of desired formants, taking the sampling frequency \(f_s\) into account (see above).
  \item
    \texttt{Window\ length} determines the time in seconds of the analysis window; the acoustic filter is assumed to be stable during this time, while the window contains several periods of the speech signal. \texttt{Praat}'s default value of 0.025 s is usually OK.
  \item
    \texttt{Time\ step} determines by how much the LPC analysis window is shifted between consecutive steps (good values are 0.005 or 0.010 s).
  \item
    \texttt{Pre-emphasis\ frequency}: This emphasis filter flattens the downward slope of the spectral envelope, so that spectral information in the higher frequencies is boosted (so that the higher frequencies are well represented in the output LPC coefficients). Here you can set the cutoff frequency of this filter; it's best to leave this at 50 Hz (somewhat below the lowest speech frequency). For more background, see §\ref{sec:emphasisfilters}.
  \end{itemize}
\end{itemize}

Then click OK. This will result in an LPC object: a series of LPC spectra, one for each frame (at \texttt{time\ step} intervals and with the chosen frame \texttt{length}).

\begin{itemize}
\tightlist
\item
  Remember to \texttt{Save} the LPC object if you wish.
\end{itemize}

\subsection{Formants}\label{formants}

From the LPC object, you can extract formant information by choosing \texttt{To\ Formant}.

\begin{itemize}
\item
  You may then \texttt{Draw} or \texttt{Tabulate} or interactively \texttt{Query} the resulting formant values (frequencies and bandwidths per formant per time frame).
\item
  Remember to \texttt{Save} the Formant object if you wish.
\end{itemize}

\subsection{Spectral slice}\label{sec:LPC-spectralslice}

From the LPC object, you can also extract a spectral slice (a spectrum at a particular time) by choosing \texttt{To\ Spectrum\ (slice)...} Use the same values for deemphasis as you chose for \texttt{preemphasis} in the analysis.
...
\tightlist
\item
  You may then \texttt{Draw} this spectrum (spectral slice), perhaps combined with the spectrum of the input signal, as in Figure \ref{fig:a8000lpc} above (in blue). Or you can inspect it interactively (\texttt{View\ \&\ Edit}) as any spectrum.
\end{itemize}

\subsection{Extract residual}\label{extract-residual}

To check the LPC analysis, you may compute the \emph{residual} from the original sound and the LPC spectrum (see §\ref{sec:inversefiltering} above).
Select both the Sound and the LPC object (use Command+click to select two objects).
Then choose \texttt{Filter\ (inverse)...}
...
This Dutch sentence may have been adapted from ``wat is het leven fijn als de zon schijnt'', from the Dutch song \emph{Als de zon schijnt} (\url{https://songteksten.net/lyric/113/66239/andre-van-duin/als-de-zon-schijnt.html}, 1983).
...
\chapter{Lexical sets}\label{app-lexicalsets}

Lexical sets are sets of words that share the same vowel, in a particular variety or accent of English.
The concept was developed by John Wells (\citet{Wells_1982}).
Each set is named after a representative keyword from that set.

For example, one lexical sets contains the words \emph{fleece, creep, speak, leave, feel, key, people}.
All words in this lexical set are spoken with the vowel /iː/ in the RP variety of British English, and with /i/ in the GA variety of American English. This lexical set is termed the FLEECE set, and the vowel is often called `the FLEECE vowel'\footnote{The lexical set keyword is typically printed in small capitals, which however are not available in the present environment.}.

The lexical sets allow easier discussion of accents and varieties of English. Within a single variety of Englishs, the lexical sets also allow easier reference to particular vowels in that variety, without using the International Phonetic Alphabet. In order to describe `the vowel in the word \emph{dance} in General American English' we can easily say `the BATH vowel'. This also expresses the generalization that this vowel is typically realized as /æ/ in General American English and as /ɑː/ in RP British English.

The full lexical sets are available in \citet{Wells_1982} and at
\url{https://en.wikipedia.org/wiki/Lexical_set}.
