...
\author{true}
\date{Document compiled 13 Jul 2025 23:07}
...
\phantomsection\label{box-soundeditor-3}
\begin{itemize}
\item
  Select an input Sound object in the \texttt{Praat} Objects window.
\item
...
\phantomsection\label{box-windowsfile-details-again}
...
\phantomsection\label{box:clearinfo}
...
\phantomsection\label{box-windowsfile-details-second}
...
\phantomsection\label{box-windowsfile-details-third}
...
\emph{Prosody} is the collective term for the `musical' properties of speech, which are typically established not by the sequence of speech sounds, but over larger units of speech (\citet{Nooteboom_1997}): these are also termed \emph{suprasegmental} properties (or, \emph{suprasegmentals} as a plural noun, \citet{Lehiste_1970}). Among these prosodic properties are:\\
- the distribution of \textbf{pauses},\\
- \textbf{tone} and \textbf{intonation} of voice pitch (§\ref{sec:frequency}, §\ref{sec:FTintro}),\\
- \textbf{durations} of speech sounds (§\ref{sec:howtoannotate}) and speech \textbf{tempo},\\
- \textbf{intensity} and loudness (§\ref{sec:amplitude}),\\
- \textbf{stress} or prominence,\\
- \textbf{accentuation} or emphasis of important words,\\
- \textbf{rhythm},\\
- and \textbf{voice quality}.

Broadly speaking, these properties of everyday speech are seldom captured in regular writing, and we need additional symbols such as \emph{punctuation} (\citet{Parkes_1992}), \emph{emoji} and \emph{accented letters} to capture these phonetic properties in writing. (In itself, the existence and widespread use of these additional symbols illustrate the importance of prosody in speech communication.)

Unfortunately, there are no simple 1:1 relations between prosodic and acoustic properties. For example, in some languages such as English and Dutch, one syllable in a polysyllabic word carries more \emph{stress} than the others: this is conveyed by segment duration, loudness, as well as pitch, in varying degrees of importance. Similarly, \emph{phrasing} is achieved by means of pausing, intonation, and/or tempo changes. And \emph{rhythm} is established by intricate combination of timing (durations) and prominence patterns. In the other direction, the duration of a vowel depends on many linguistic and phonetic factors, such as its position within its phrase and word and syllable, the stress status of the syllable, the accent status of the word, and more (\citet{Klatt_1976}).
...
\section{Pauses and tempo}\label{pauses-and-tempo}

Typical speech contains \textbf{pauses}, which may serve multiple functions: (1) pauses are used to demarcate linguistic units in what is said, (2) pauses occur naturally whenever speakers breathe in fresh air, (3) speakers may pause their own speech if they need extra preparation time for what they wish to say, (4) rhetorical purposes.\\
In practice, these functions may interfere: for example, a speaker may pause \emph{within} a linguistic unit (contra 1), perhaps to select the appropriate word (see Fig.\ref{fig:s344f5-pausing} for an example). Or a speaker may produce a filled pause (contra 2) for some rhetorical or communicative purpose.
...
The lower threshold for a speech pause is often chosen to be 0.25 seconds (\citet{DeJong-Bosker-2013}).
...
{\centering \includegraphics{figures/s344f5_formal_fragment} 

}

\caption{Oscillogram and annotation of the sentence *Yeah you can you can really see how.... how diff'rent th'approaches of the students involv'd were.*}\label{fig:s344f5-pausing}
\end{figure}

\phantomsection\label{box-s344f5_pausing}
The oscillogram and annotation in Fig.\ref{fig:s344f5-pausing} shows a spoken sentence, taken from the interview recording named \texttt{s344f5\_1.wav} from the LUCEA corpus. The sentence is taken from the formal monologue part of the interview. For details about the corpus, see \citet{Orr_Quené_2017}. Some vowels are strongly reduced, as indicated in the caption. The first phrase contains 9 realized syllables, and the second phrase 13.
...
\subsection{Pausing}\label{pausing}

\subsection{Tempo}\label{tempo}

From Table X we may calculate that the \textbf{speaking rate} or \textbf{speech rate} or \textbf{tempo} (including pause time) is 21 syllables in 6.974 seconds, or 3.0 syllables per second. The \textbf{articulation rate} (excluding pause time) is 21 syllables in 5.262 seconds, or 4.0 syllables per second.

\section{Durations}\label{durations}

\section{Frequency (pitch)}\label{frequency-pitch}

\section{Intensity (loudness)}\label{intensity-loudness}

\section{Stresses and accents}\label{stresses-and-accents}

\section{Rhythm}\label{rhythm}

\section{Voice quality}\label{voice-quality}
