...
\date{Document compiled 05 Dec 2024 12:54}
...
The target-to-reference ratio is typically expressed on a logarithmic scale, in \textbf{decibel} units (dB), named after Alexander Graham Bell (1847-1922, inventor of the telephone).
...
\item
  ...
  While the reference \(p_0\) is expressed in RMS units of sound pressure (in Pascal units), ratios using \(p_0\) as absolute reference are termed \texttt{dB\ Sound\ Pressure\ Level} or \texttt{dB(SPL)}, see Table \ref{tab:decibels} below.\\
...
Select an input Sound object in the Praat Objects window. Then choose \texttt{Filter\ \textgreater{}\ Filter\ (pre-emphasis)...} for pre-emphasis filtering, or choose \texttt{Filter\ \textgreater{}\ Filter\ (de-emphasis)...} for de-emphasis filtering. The filter only applies above a certain cutoff frequency, to be specified. Choose a cutoff frequency below the lowest speech frequency, e.g.~70 Hz.\\
