...
\date{Document compiled 18 Nov 2024 16:32}
...
A periodic sound wave can be characterized by three key properties, which are illustrated in the oscillogram below and which are further discussed in the following sub-sections.
...
\begin{quote}
TODO: insert here a picture of a waveform in the space domain, with distance along X axis (not an oscillogram with time along X axis), illustrating wave length, at a fixed point in time (single ``snapshot'' of air pressure over distance).
