...
\date{Document compiled 13 Nov 2024 12:25}
...
\begin{itemize}
\item
  \textbf{periodic} sounds, in which there is sound wave pattern that repeats itself after a particular \textbf{period} or time interval (symbol \(T\)) of a single cycle. Periodic sounds have a perceptible pitch or tone. The vowel sound for /i/ in Figure \ref{fig:speech-oscillogram} (from 0.177 to 0.238 s) provides a clear example of a periodic sound.
\item
  \textbf{aperiodic} sounds, in which there is not a repetitive but a random pattern in the air pressure variations. Aperiodic sounds do not have a perceptible pitch but instead we hear them as noise. The consonant sound for /s/ in Figure \ref{fig:speech-oscillogram} (from 0 to 0.099 s) provides a clear example of a noisy, aperiodic sound.
\end{itemize}

\section{Frequency}\label{frequency}

The frequency (symbol \(f\)) of a sound wave is the number of repeated cycles or periods (of air pressure variations) within a time interval. Only periodic sounds do have such repetitions, and thus a frequency. The frequency of a sound is perceived as its \emph{pitch} or tone. Frequency is expressed in Hertz units, that is, in periods per second (in older texts you may find `cycles per second', abbreviated `cps'). Each of these periods or cycles corresponds to the repeating fluctuation between two consecutive maxima, or between corresponding `zero crossings' of adjacent periods. In Figure \ref{fig:speech-oscillogram}, in the vowel /i/, we count 14 periods in 0.065 seconds, so \(f \approx 14/0.065 \approx 215\) Hz. These periods have a duration of about \(T \approx 0.065/14 \approx 0.0046\) s\footnote{More exactly, the period \emph{is} 0.0046 s.}. Period \(T\) and frequency \(f\) are each other's inverse, so \(f=1/T\) and \(T=1/f\).
