...
\date{Document compiled 16 Dec 2024 17:53}
...
\subsection{\texorpdfstring{How to obtain an entire Spectrum in \texttt{Praat}}{How to obtain an entire Spectrum in Praat}}\label{how-to-obtain-an-entire-spectrum-in-praat}
...
\phantomsection\label{box-praatspectrum}
\subsection{\texorpdfstring{How to obtain a spectral `slice' in \texttt{Praat}}{How to obtain a spectral `slice' in Praat}}\label{how-to-obtain-a-spectral-slice-in-praat}

Only rarely are we interested in the spectrum of an entire Sound object. Typicall, we want to inspect the spectrum of the sound only over a small time window within that sound. For example, we might want to inspect the spectrum of the vowel sound in Figure \ref{fig:speech-oscillogram} at \(t \approx 0.210\) seconds.

This can be done in \texttt{Praat} by means of two features we will explore in depth only later, viz.~the SoundEditor and the Spectrogram.

\begin{quote}
TODO crossref SoundEditor and crossref Spectrogram
\end{quote}
...
\begin{itemize}
\item
  In the SoundEditor window, go to \texttt{Spectrogram...} and then \texttt{Spectrogram\ settings}. Here we may need to adjust the setting for \texttt{Window\ length}. \texttt{Praat} will average the resulting spectrum over the time window of \(2\times\) this value, centered around the position of the cursor in the oscillogram.\\
  A short window (e.g.~0.005 s) will show more detail in the time domain, but the resulting spectrum will be smeared in the frequency domain (so that individual harmonics may be invisible). A long window (e.g.~0.015 s) will show less detail in the time domain (so that individual periods and clicks will be smeared in the time domain), but more detail in the frequency domain (so that individual harmonics may be visible). Read the \texttt{Praat} Help information available in the menu window, try various window lengths, and notice the differences in the subsequent spectral slices.
\item
...
\caption{Spectral slice of the /i/ vowel in the word *speech*, estimated over 0.015 s on either side of the cursor at t=0.210 s. With this window length, individual harmonics are clearly visible in the resulting spectrum.}\label{fig:speech-spectralslice}
\end{figure}

\begin{itemize}
\tightlist
\item
  In Fig.\ref{fig:speech-spectralslice}, the 10th harmonic is at 2195 Hz, which suggests that \(f_0 \approx 219\) Hz (see §\ref{sec:FTintro}). Also notice the overall downward slope of the spectrum (see §\ref{sec:emphasisfilters} and §REF).
\end{itemize}
