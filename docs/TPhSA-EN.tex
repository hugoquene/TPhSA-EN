...
\author{true}
\date{Document compiled 25 Jan 2025 00:40}
...
\item
  ...
  Figure Fig.\ref{fig:window-soundeditor} shows an example of a SoundEditor, displaying the fragment \emph{\ldots a small window to get this\ldots{}} spoken by a male speaker of American English; the single word \emph{window} is highlighted.
\end{itemize}
...
The SoundEditor contains a lot of information! On the top you always see an oscillogram (§\ref{sec:oscillogram}). Below, there are additional and overlapping visualisations of the Spectrogram (Ch.\ref{ch-spectrograms}), Pitch, Intensity, Formants (§\ref{sec:formants}), and voice Pulses. These additional visualisations will be discussed later in this tutorial guide; for now, you may switch them \texttt{off} by choosing the appropriate buttons in the top of the SoundEditor window, and then unselecting the \texttt{Show} option.
...
\item
  You can play back the selected part, or the parts before/after it, by clicking the appropriate buttons (labeled with the durations of those parts). For more information about audio playback from \texttt{Praat}, consult §\ref{sec:playback} above.
\item
...
\tightlist
\item
  To save the selected part as a separate audio file: in the SoundEditor, choose \texttt{File\ \textgreater{}\ Save\ selected\ sound\ as\ WAV\ file...}.
