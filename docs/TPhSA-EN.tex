...
\author{true}
\date{Document compiled 28 Dec 2024 10:31}
...
\caption{Uniform straight tube of length L. After Stevens (1998), Figure 3.8.}\label{fig:onetube}
\end{figure}
...
The natural frequencies resonate (are amplified) because their wavelength matches the length \(L\) of the tube: the compressions and rarefactions of one period coincide with those of the next period in time, so that the amplitudes of the pressure variations add up, and thus increase for the resonant frequencies: the resonant frequencies are amplified and other frequencies are attenuated\footnote{Around its natural frequencies, the tube acts as a bandpass filter (albeit with a filter characteristic different from the one illustrated in Fig.\ref{fig:bandpassfilter}).}.
...
\caption{Three-tube configuration, with a back cavity (left), constriction, and front cavity (right). After Johnson (2003), Figure 5.3.}\label{fig:threetube}
\end{figure}
...
\item
  As we saw above, for a male speaker we may expect 5 formants in the range from 0 to 5000 Hz. For a female speaker, having a slightly shorter vocal tract than a male speaker, we may expect 5 formants in the range from 5 to 5500 Hz. Choose appropriate \texttt{Maximum\ number\ of\ formants} and matching \texttt{Formant\ ceiling} values for your speaker; these values depend on the length of the speaker's vocal tract and on your sampling frequency (§\ref{sec:samplingfrequency})\footnote{See the \texttt{Help} information here, in particular its last paragraph.}.
...
\begin{quote}
TODO discuss source-filter-theory here
\end{quote}
