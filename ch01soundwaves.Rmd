...
- **periodic** sounds, in which there is sound wave pattern that repeats itself after a particular **period** or time interval (symbol $T$) of a single cycle. Periodic sounds have a perceptible pitch or tone. The vowel sound for /i/ in Figure \@ref(fig:speech-oscillogram) (from 0.177 to 0.238 s) provides a clear example of a periodic sound. 

- **aperiodic** sounds, in which there is not a repetitive but a random pattern in the air pressure variations. Aperiodic sounds do not have a perceptible pitch but instead we hear them as noise. The consonant sound for /s/ in Figure \@ref(fig:speech-oscillogram) (from 0 to 0.099 s) provides a clear example of a noisy, aperiodic sound. 

## Frequency

The frequency (symbol $f$) of a sound wave is the number of periods (of air pressure variation) within a time interval. Only periodic sounds do have a frequency, which is perceived as the pitch of a sound. Frequency is expressed in Hertz units, that is, in periods per second (or cycles per second, cps). Each of these periods or cycles corresponds to the repeating fluctuation between two consecutive maxima, or between corresponding 'zero crossings' of adjacent periods. In Figure \@ref(fig:speech-oscillogram), in the vowel /i/, we count 14 periods in 0.065 seconds, so $f\approx 14/0.065 \approx 215$ Hz. These periods have a duration of about $T \approx 0.065/14 \approx 0.0046$ s^[More exactly, the period *is* 0.0046 s.]. Period $T$ and frequency $f$ are each other's inverse, so $f=1/T$ and $T=1/f$. 
