...
In practice, the sound pressure or intensity of a target sound is compared with some reference sound pressure or intensity, and the target-to-reference ratio is then expressed on a logarithmic scale.
The target-to-reference ratio is typically expressed on a logarithmic scale, in **decibel** units (dB), named after Alexander Graham Bell (1847-1922, inventor of the telephone). 
...
While the reference $p_0$ is expressed in RMS units of sound pressure (in Pascal units), ratios using $p_0$ as absolute reference are termed `dB Sound Pressure Level` or `dB(SPL)`, see Table \@ref(tab:decibels) below.\
