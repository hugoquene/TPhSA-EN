...
The SoundEditor in Fig.\@ref(fig:window-soundeditor) shows a fragment of speech, taken from the audio file named `s321f1-1.wav` from the Longitudinal UCU English Accents corpus (LUCEA; @Orr_Quené_2017). The utterance shown is taken from the informal monologue part of the recording. 
...
In `Praat`, each new writing action such as `Query`, `Get`, `Tabulate`, etc, first **clears** the Info window, thus erasing any contents in the Info window. If you wish to keep information from the Info window, you have to `Save` or export its contents *before* the next writing action!

You can also clear the Info window manually, by choosing `File > Clear` in the Info window.  
