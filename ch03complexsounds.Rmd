...
*Chapter keywords*: sinewave sound, complex sound, spectrum, fourier transform, fourier analysis, fast fourier transform, FFT, harmonics, fundamental, fundamental frequency, f0, pitch, overtone, component, timbre, octave, noise, white noise, brown noise, impulse. 
...
- *attack* (A): the "onset time" during which the property increases after the key has been pressed (in case of a piano key: the time between key-press and the hammer hitting the piano string and the string then starting to produce sound);

- *decay* (D): the time during which the property falls back from the peak of the attack (for a piano: the time from the instant of maximum sound) to the ... 

- *sustain* (S): the _level_ of the property after the attack and decay phases, while the key is still pressed (for a piano: the level of sound from the string vibrating shortly after it was hit);

- and *release* (R): the time during which the property falls back from the sustain level, _after_ the key has been released (for a piano: very brief, unless the pedal is used).
