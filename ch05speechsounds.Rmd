...
```{r onetube, echo=FALSE, fig.cap="Uniform straight tube of length L. After Stevens (1998), Figure 3.8.", fig.align="center"}
...
The natural frequencies resonate (are amplified) because their wavelength matches the length $L$ of the tube: the compressions and rarefactions of one period coincide with those of the next period in time, so that the amplitudes of the pressure variations add up, and thus increase for the resonant frequencies: the resonant frequencies are amplified and other frequencies are attenuated^[Around its natural frequencies, the tube acts as a bandpass filter (albeit with a filter characteristic different from the one illustrated in Fig.\@ref(fig:bandpassfilter)).].
...
- As we saw above, for a male speaker we may expect 5 formants in the range from 0 to 5000 Hz. For a female speaker, having a slightly shorter vocal tract than a male speaker, we may expect 5 formants in the range from 5 to 5500 Hz. Choose appropriate `Maximum number of formants` and matching `Formant ceiling` values for your speaker; these values depend on the length of the speaker's vocal tract and on your sampling frequency (§\@ref(sec:samplingfrequency))^[See the `Help` information here, in particular its last paragraph.]. 
